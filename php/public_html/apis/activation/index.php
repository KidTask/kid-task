...
require_once("/etc/apache2/kidtask/Secrets.php");
...
try{
	...
	if($method === "GET"){
		...
		$adult = Adult::getAdultByAdultActivationToken($pdo, $activation);

		//verify the profile is not null
		if($adult !== null){

			//make sure the activation token matches
			if($activation === $adult->getAdultActivationToken()) {
			...
			throw(new RuntimeException("Account with this activation code does not exist", 404));
		}
	} else {
